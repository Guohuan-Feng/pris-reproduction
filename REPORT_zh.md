...
在固定 PSS 截断值 −0.6368790173149083 下，260 个选定候选的保留率随所采用的性质阈值变化：
...
约 99.2% 的保留率对应映射后的阈值，其计算为 400 × median(B_DFT/B_UMA) ≈ 375.81874 GPa。达到该阈值并被保留的 123 个候选包含 104 个 UMA-priority 组候选和 19 个 control 组候选。第 4.4 节区分了这一选定队列的 DFT 结果与完整候选池的筛选计数。

重拟合 DFT 体模量超过 400 GPa 的两个候选为 `candidate_0017`（Os，priority 组，约 400.38451 GPa）和 `candidate_0980`（Re2IrOs6，screened 组，约 418.54561 GPa）。后者在这 260 个候选中具有最高体模量，且属于筛除组。
...
### 4.4 67.3% 与 99.2% 摘要结果的解释

[论文摘要](https://arxiv.org/html/2609.01209v1) 确实将队列最多缩减 67.3% 与保留 99.2% 并列呈现为 PRIS 和 PSS 的结果。因此，同时报告两个百分比忠实反映了摘要表述。复现层面的解释还需明确各百分比对应的筛选策略、受评估队列和性质阈值。

固定版本的 [STATUS.json](https://github.com/AI4QC/PRIS/blob/34e6c86c083759dc1ee594ae22238ea9b5ebd8f4/outputs/20260823_fig45_merged_nature_inverse_f_v1/STATUS.json) 与 [E4 队列扫描记录](https://github.com/AI4QC/PRIS/blob/34e6c86c083759dc1ee594ae22238ea9b5ebd8f4/dft/E4_design/queue_sweep.json) 区分了以下评估。固定 PSS 截断值为 c = −0.6368790173149083，低于 c 的分数对应筛除。

| 筛选策略与队列 | 队列缩减 | 定义保留率分母的性质条件 | 保留 / 达标 |
|---|---|---|---:|
| Set 4；完整 1,081 个候选池 | 728/1,081（67.345%） | UMA 体模量代理值 ≥400 GPa | 45/140 |
| PSS 截断值 c；完整 1,081 个候选池 | 61/1,081（5.643%） | UMA 体模量代理值 ≥400 GPa | 140/140 |
| 相同 PSS 截断值 c；选定的 260 个 DFT 候选 | 不从该队列推断完整候选池的缩减率 | DFT 体模量 ≥400 × median(B_DFT/B_UMA) ≈375.81874 GPa | 123/124（99.19355%） |

前两行记录作者发布的聚合计数，未对完整候选池独立重新评分。第三行通过独立重拟合公开 E(V) 数据核验。其保留的 123 个候选由 104 个 UMA-priority 候选和 19 个 control 候选组成。因此，UMA-priority 的 140 个分母与经偏差校正后 DFT 达标的 124 个分母对应不同选集。

据此，67.3% 对应 Set 4 的队列缩减操作点，而 99.2% 对应固定 PSS 截断值在选定、经偏差校正的 DFT 评估中的保留率。同一 PSS 截断值对应的公开完整候选池缩减率为 5.643%。这些区别明确了摘要中并列表述的适用范围：现有发布记录尚未确立一个固定操作点，能够针对同一受评估候选池，同时实现 67.3% 的总体队列缩减与 99.2% 的性质达标候选保留率。原始绝对 DFT 目标 400 GPa 仍为单独的判断条件，其结果见第 3.3 节。
